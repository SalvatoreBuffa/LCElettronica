...
/**
 * Classe che modellizza la struttura della tabella contenente le informazioni sui clienti di tipo azienda
...
public class CompanyTableModel extends ItemTableModel {
     ...
     */
    public CompanyTableModel(CustomerManager manager){
        ...
        //aggiungiamo la lista di clienti aziende alle righe
        for(Customer c: customers){
            if(c.getType().equals(SettingName.COMPANYTYPE)){
                rows.add(c);
            }
        }
    }
    ...
    private void createFirstRow(){
        columns.add(SettingName.COMPANYNAME);
        columns.add(SettingName.CUSTOMERPIVA);
        columns.add(SettingName.COMPANYCODESDI);
    ...
    public void update(int oldID, Item newItem) {
        rows.remove(manager.searchElement(oldID));
        manager.update(oldID, (Customer) newItem);
        rows.add(manager.searchElement(oldID));
        fireTableDataChanged();
    }
    ...
    public void importCSV(String filePath) {
        manager.importData(filePath, SettingName.COMPANYTYPE);
        this.customers = CustomerManager.getList();
        rows.clear();
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        switch (columnIndex){
            case (0):
                return i.get(SettingName.COMPANYNAME);
            case (1):
                return i.get(SettingName.CUSTOMERPIVA);
            case (2):
                return i.get(SettingName.COMPANYCODESDI);
            case (3):

...
/**
 * Classe che crea il pannello contenente la lista di prodotti memorizzati ed i vari bottoni per eseguire operazioni su di essi
...
public class ProductPanel extends JPanel {
    ...
    public ProductPanel(){
        ...
        List<Product> list = ProductBackup.restoreList();
        ...
        listPanel = new JScrollPane(renderProductTable());
     ...
     */
    private JTable renderProductTable(){
        //creiamo il modello
        model = new ProductTableModel(manager);
    ...
    private void renderOperationButtons(){
        //creiamo il bottone "Inserisci" che al click apre il frame per inserire un prodotto
        addButton = ButtonFactory.createButton(SettingName.GUIINSERT, e -> {
            ProductInsertFrame.buildFrame(model);
        });
        ...
        exportButton = ButtonFactory.createButton(SettingName.GUIEXPORT, e -> {
            ...
            picker.setMode("save");
            manager.exportData(picker.getPath()+"\\products.csv");
        });
        exportButton.setEnabled(!ProductManager.getList().isEmpty());
        ...
        //creiamo il tasto "Annulla operazione" che al click ripristina lo stato dei prodotti all'operazione precedente
